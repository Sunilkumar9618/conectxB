...
function updateMobilePreview() {
    // Update profile section in mobile mockup
    ...
    // Find the phone mockup elements (they're in a .phone-content-wrapper)
    const phoneContent = document.querySelector('.phone-content');
    if (!phoneContent) return;
    
    // Get all elements in the header section (first padding div)
    const headerDiv = phoneContent.querySelector('div[style*="padding: 24px"]');
    if (!headerDiv) return;
    
    // Update name (h2 element)
    const nameDisplay = headerDiv.querySelector('h2');
    if (nameDisplay) {
        nameDisplay.textContent = nameInput.value || 'Your Name';
    }
    
    // Update bio (p element with opacity)
    const paragraphs = headerDiv.querySelectorAll('p');
    if (paragraphs.length >= 2) {
        paragraphs[paragraphs.length - 1].textContent = bioInput.value;
    }
    
    // Update avatar
    const avatarDiv = headerDiv.querySelector('div[style*="width: 100px"]');
    if (avatarDiv) {
        const avatars = getFromStorage('beacons_avatars', {});
        const avatar = avatars[handle];
        if (avatar) {
            ...
            avatarDiv.textContent = getInitials(nameInput.value || 'User');
        }
    }
    
    // Update blocks section
    const userTheme = builder.theme || { bg: '#667eea', btn: '#667eea', textColor: '#ffffff' };
    const visibleBlocks = builder.blocks.filter(b => b.type !== 'social' && b.visible);
    
    // Find the blocks container (the last major section in phone-content)
    const allDivs = phoneContent.querySelectorAll('div[style*="padding"]');
    let blocksContainer = null;
    
    // Find the div that contains the empty state or blocks
        ...
        avatarUpload.addEventListener('change', (e) => {
            ...
            if (file) {
                ...
                reader.onload = (event) => {
                    ...
                    const avatars = getFromStorage('beacons_avatars', {});
                    avatars[handle] = avatarData;
                    saveToStorage('beacons_avatars', avatars);
                    
                    // Update profile in Firebase (no avatar there)
                    // Just save to localStorage
                    const users = getFromStorage('beacons_users', {});
                    if (users[handle]) {
                        users[handle].avatar = avatarData;
                        saveToStorage('beacons_users', users);
                    }
    ...
    document.querySelectorAll('.block-action-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (action === 'edit') {
                ...
                builder.saveBlocks();
                updateMobilePreview();
                showToast('Block visibility toggled', 'info', 1500);
            } else if (action === 'delete') {
                if (confirm('Delete this block?')) {
                    builder.deleteBlock(blockId);
                    builder.saveBlocks();
                    updateMobilePreview();
                    showToast('Block deleted', 'success', 1500);
                }
            }
        });
    });
        ...
        blocksList.addEventListener('drop', (e) => {
            ...
            if (builder.draggedBlock) {
                ...
                if (targetCard) {
                    ...
                    builder.saveBlocks();
                    updateMobilePreview();
                    showToast('Block reordered', 'success', 1500);
                }
            }
        });
...
function showAddBlockModal() {
    ...
    window.addBlockType = (type) => {
        ...
        modal.close();
        updateMobilePreview();
        showToast('Block added ✓', 'success', 1500);
    };
        ...
        actions: [
            { label: 'Cancel', type: 'ghost', callback: () => {} },
            {
                ...
                callback: () => {
                    ...
                    builder.saveBlocks();
                    updateMobilePreview();
                    showToast('Block updated ✓', 'success', 1500);
                    modal.close();
                }
            }
        ]
