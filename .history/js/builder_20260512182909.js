...
function updateMobilePreview() {
    ...
    const nameDisplay = document.querySelector('.builder-user-name');
    const bioDisplay = document.querySelector('.builder-user-bio');
    const avatarPreview = document.querySelector('.builder-avatar-preview');
    
    if (nameDisplay && nameInput) {
        nameDisplay.textContent = nameInput.value || 'User';
    }
    ...
    if (avatarPreview) {
        ...
        const avatar = avatars[handle];
        if (avatar) {
            avatarPreview.innerHTML = `<img src="${avatar}" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">`;
        } else {
            avatarPreview.textContent = getInitials(nameInput?.value || 'User');
        }
    }
    
    // Update blocks preview
    const blocksList = document.getElementById('blocks-list');
    if (blocksList) {
        const blockPreviews = blocksList.querySelectorAll('.block-preview');
        blockPreviews.forEach((preview, index) => {
            if (builder.blocks[index]) {
                preview.innerHTML = renderBlockPreview(builder.blocks[index]);
            }
        });
    }
}
