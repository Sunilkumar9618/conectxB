...
views.publicPage = async function(handle) {
    ...
    try {
        ...
        if (typeof fbDb !== 'undefined') {
            ...
            if (!userDoc.empty) {
                ...
                const blocksDoc = await fbDb.collection('blocks').where('userId', '==', user.uid).get();
                blocks = blocksDoc.docs.map(doc => ({ id: doc.id, ...doc.data() }));
                
                // Load products from Firestore
                const productsDoc = await fbDb.collection('products').where('userId', '==', user.uid).get();
                products = productsDoc.docs.map(doc => ({ id: doc.id, ...doc.data() }));
                
                // Load theme from Firestore
                const themeDoc = await fbDb.collection('themes').doc(user.uid).get();
                if (themeDoc.exists) {
                    userTheme = themeDoc.data();
                }
            }
        }
    } catch (error) {
    ...
    if (!user) {
        ...
        // Load avatar from localStorage for Firestore users too
        const avatars = getFromStorage('beacons_avatars', {});
        avatar = avatars[handle] || '';
    }
...
window.getSocialIconsSVG = function(platforms) {
    const socialIcons = {
        ...
        twitter: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 002.856-3.715v-.736H20.692a10.003 10.003 0 01-9.058 4.017c-3.118 0-5.95-1.584-5.95-3.53 0-.405.092-.779.275-1.123A4.994 4.994 0 002.988 4.25c-.956-.379-1.951-.64-2.988-.767v.643c0 2.987 2.144 5.265 5.028 5.887-.092.213-.146.45-.146.693 0 .41.123.81.365 1.15-.407-.01-.802-.04-1.189-.1.336 1.173 1.499 2.027 2.868 2.087-.527.645-1.195 1.135-1.993 1.26.784.236 1.694.457 2.656.457 3.107 0 4.744-.993 5.744-1.942.71-.71 1.217-1.5 1.58-2.318-.5.639-1.2 1.319-2.041 1.82.02.105.038.211.038.32 0 .755-.244 1.44-.67 2.02.426-.584.7-1.274.7-2.02 0-.105-.018-.212-.037-.316.84-.504 1.541-1.184 2.04-1.82.363.82.87 1.61 1.58 2.32 1 .95 2.636 1.94 5.743 1.94.962 0 1.872-.22 2.656-.456.8-.126 1.466-.614 1.993-1.26 1.369-.06 2.532-.913 2.868-2.087-.388.06-.782.09-1.189.1.242-.34.365-.74.365-1.15 0-.244-.055-.48-.146-.694 2.884-.622 5.028-2.9 5.028-5.886v-.643c-1.037.127-2.032.388-2.988.767z"/></svg>',
        instagram: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0m5.521 17.05c-.0861.2 3.708-6.633.02-9.566-1.409-1.14-3.152-1.45-5.541-1.45s-4.132.31-5.541 1.45c-3.688 2.933.106 9.766.02 9.566-1.16.254-2.415.254-3.575.064-1.084-.180-2.1-.69-2.986-1.575.886.885 1.902 1.395 2.986 1.575 1.16.19 2.415.19 3.575-.064.2-.086 6.633 3.708 9.566.02 1.14-1.409 1.45-3.152 1.45-5.541s-.31-4.132-1.45-5.541c-2.933-3.688-9.766.106-9.566.02-.254-1.16-.254-2.415-.064-3.575z"/></svg>',
        linkedin: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.475-2.236-1.986-2.236-1.081 0-1.722.722-2.004 1.418-.103.25-.129.599-.129.948v5.439h-3.554s.05-8.736 0-9.646h3.554v1.364c.43-.665 1.199-1.61 2.922-1.61 2.134 0 3.732 1.388 3.732 4.369v5.523zM5.337 8.855c-1.144 0-1.915-.762-1.915-1.715 0-.953.77-1.715 1.96-1.715 1.188 0 1.914.762 1.939 1.715 0 .953-.751 1.715-1.984 1.715zm1.946 11.597H3.392V9.806h3.891v10.646zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.225 0z"/></svg>',
        youtube: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>',
        tiktok: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v19.34a2.86 2.86 0 0 1-5.08 1.5A2.859 2.859 0 0 1 5.96 15.66a2.84 2.84 0 0 1 5.07-1.5v-3.4a6.27 6.27 0 0 0-5.85 4.11V9.4a6.6 6.6 0 0 0-1.63-.2A6.6 6.6 0 0 0 .4 16a6.6 6.6 0 1 0 13.16 0 6.6 6.6 0 0 0-.63-2.31z"/></svg>',
        whatsapp: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.198.297-.768.966-.941 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.67-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.076 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421-7.403h-.004a9.87 9.87 0 00-5.031 1.378l-.361.214-3.741-.982.998 3.645-.235.374a9.861 9.861 0 001.51 5.694c.33.474.923 1.99 2.263 3.286 1.922 1.925 4.827 2.924 7.287 2.207 2.169-.65 4.038-2.586 4.849-4.774.464-1.159.674-2.432.674-3.782 0-.245-.007-.49-.021-.734-.168-.951-.595-1.81-1.242-2.583-.491-.6-1.169-1.149-1.933-1.573-.404-.23-.768-.435-1.161-.573.154-.023.31-.036.464-.036 1.227 0 2.349-.408 3.172-1.091.535-.441 1.009-1.045 1.327-1.704.222-.475.379-.994.474-1.534.049-.274.068-.545.068-.808 0-.796-.166-1.55-.482-2.234-.41-.905-1.049-1.712-1.863-2.285-.897-.648-1.946-.98-3.015-.98-.588 0-1.168.067-1.735.2"/></svg>',
        email: '<svg class="social-icon-svg" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>'
    };
    
    return Object.entries(platforms).map(([key, url]) => {
        if (!url) return '';
        const icon = socialIcons[key] || socialIcons.email;
