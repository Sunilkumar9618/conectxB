            ...
            try {
                ...
                firebaseService.onAuthStateChanged(async (user) => {
                    if (user) {
                        ...
                        console.log('📋 User profile:', this.currentUser.profile);
                    } else {
...
async function handleSignup() {
    ...
    // Success
    // Save session to localStorage with loggedIn flag
    const sessionData = {
        uid: auth.currentUser.uid,
        email: auth.currentUser.email,
        handle: auth.currentUser.profile?.handle,
        name: auth.currentUser.profile?.name,
        loggedIn: true
    };
    setToStorage('jsbeacons_session', sessionData);
